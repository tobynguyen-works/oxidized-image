...
Install the integration and its peer dependencies using your preferred package manager:

```bash
npm install @oxidized-image/astro @napi-rs/image
```

```bash
pnpm add @oxidized-image/astro @napi-rs/image
```

```bash
yarn add @oxidized-image/astro @napi-rs/image
```

> **Note:** `@napi-rs/image` is a required peer dependency and must be installed alongside `@oxidized-image/astro`.
