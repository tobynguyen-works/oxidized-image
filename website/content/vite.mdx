...
```bash
npm install @oxidized-image/vite @napi-rs/image
```

```bash
pnpm add @oxidized-image/vite @napi-rs/image
```

```bash
yarn add @oxidized-image/vite @napi-rs/image
```

> **Note:** `@napi-rs/image` is a required peer dependency and must be installed alongside `@oxidized-image/vite`.
